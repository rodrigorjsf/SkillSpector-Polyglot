...
- **Seven matrix rows extended in place**, each keeping upstream's original rule list and rationale
  ...
  `DA-UNRESOLVED`). Rows `AST07`, `AST09` and `AST10` are untouched upstream text.

  Two of the seven — `AST01` and `AST05` — carry a different kind of edit, and it is the fork
  writing down *upstream's* rules rather than its own: the 2.9.6 sync brought `P9`, `SC8`, `SC9`,
  `AST10`, `TT6` and `DS1`-`DS4` into the catalog while this page was held here, and upstream had
  no revision of the page to put them in. Those additions are the fork's text about upstream's
  rules, so when upstream republishes they are the *first* thing to drop in favour of whatever
  upstream's own reworded rows say.
...
OWASP uses `AST10` to name the Agentic Skills Top 10 project. SkillSpector also uses `AST1` through `AST10` as internal rule ids for the Behavioral AST analyzer family. Those names are unrelated, and since upstream's 2.9.6 added a tenth rule to that family the two vocabularies now collide on the exact string `AST10`. The zero is the discriminator: in this page `AST01` through `AST10` — zero-padded — refer to OWASP risk categories, while `AST1` through `AST10` — unpadded — refer to SkillSpector rule ids.
...
|---|---|---|---|
| AST01 Malicious Skills | `P1`-`P5`, `P9`, `AR1`-`AR3`, `E1`-`E4`, `PE3`, `SC2`, `SC3`, `SC8`, `SC9`, `MP1`-`MP3`, `RA1`, `RA2`, `AST1`-`AST10`, `TT3`-`TT6`, `DS1`-`DS4`, `YR1`-`YR4`, `TP1`-`TP3` | Related rules present | Current rules detect malicious instructions, secret theft, persistence, dangerous execution chains, known malware signatures, and poisoned metadata commonly used by malicious skills. Upstream's 2.9.6 widened the concealment surface they read: `P9` reports whitespace padding that pushes instructions out of the visible area, `SC8` and `SC9` report code shipped where a reviewer does not look — Python bytecode, and an executable nested inside a document container — and `AST10`, `TT6` and `DS1`-`DS4` report deserialization of untrusted bytes, which is arbitrary code execution written as data loading in Python, PHP, Ruby and JavaScript alike. |
